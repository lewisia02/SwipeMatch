...
export default function FinalVotePage() {
  ...
  useEffect(() => {
    ...
    async function loadKeptLogos() {
      ...
      try {
        ...
        const manager = new SwipeSessionManager(slug, logos);
  ...
  async function handleSubmit() {
    ...
    try {
      const res = await fetch(`/api/c/${slug}/votes`, {
        method: 'POST',
  ...
  return (
    <main className="mx-auto flex max-w-md flex-col gap-6 p-6">
      <div className="flex items-center gap-3">
        <Link href={`/c/${slug}/vote/swipe`} className="text-caption text-primary underline">
          ← 戻る
        </Link>
