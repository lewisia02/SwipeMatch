...
# ❌ 悪い例
data = fetch()
def calc(arr): ...  # 型ヒントなし・名前が曖昧
```

**原則**:
- 変数: camelCase（TS）/ snake_case（Python）、名詞または名詞句
- 関数: camelCase（TS）/ snake_case（Python）、動詞で始める
- 定数: UPPER_SNAKE_CASE（両言語共通）
- Boolean: `is`, `has`, `should`（Python は `is_`, `has_`, `should_`）で始める
...
```python
# Python: クラスは PascalCase、メソッド・属性は snake_case
class TaskManager: ...
class UserAuthenticationService: ...

# 型エイリアス（type）や Literal で状態を表現
