...
上記は TypeScript エコシステムの構成です。Python 側は以下のツールを用います。

1. **Lint / Format**: **ruff** — Lint（`ruff check`）とフォーマット（`ruff format`）を高速に実行。設定ファイル: `pyproject.toml`
2. **型チェック**: **mypy**（または pyright）— 静的型検査。設定ファイル: `pyproject.toml`
3. **テスト実行**: **pytest** — カバレッジは `pytest --cov`（pytest-cov）で測定
4. **パッケージ・環境管理**: **uv** — `uv sync` / `uv run` で依存と実行を管理
...
> 単一言語のプロジェクトでは、該当する `job`（`node` または `python`）のみを残してください。
...
Python を含むプロジェクトでは、pre-commit で Python 側のチェックも実行します（[pre-commit](https://pre-commit.com/) フレームワーク、または husky からの呼び出し）。
...
**この構成を選んだ理由**:
- 2025年時点での TypeScript / Python エコシステムにおける標準的かつモダンな構成
