...
- コンポーネント設計
- アルゴリズム設計（該当する場合）
- UI設計（CLI向け。Web/GUI の画面設計は `docs/ui-design.md` を参照）
- エラーハンドリング
...
### ステップ7: UI設計（CLI向け・該当する場合）

> **Web/GUI アプリケーションの画面設計**は、画面設計書 `docs/ui-design.md`（`ui-design` スキル）で定義します。機能設計書では、CLI ツールの表示設計のみを扱います。
