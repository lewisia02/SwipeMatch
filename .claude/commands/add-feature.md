...
3. **フロントエンド(UI)の変更が含まれる場合**は、視覚検証を行う。含まれない場合は本項と次項をスキップする。
  - 開発サーバーを起動し、Playwright で対象画面のスクリーンショットを取得する:
    ```bash
    npx playwright screenshot --wait-for-timeout 2000 "http://localhost:[ポート]/[パス]" ".steering/[日付]-[機能名]/screenshots/[画面ID].png"
    ```
  - 取得したスクリーンショットを `docs/ui-design.md` のワイヤーフレーム・状態設計と突き合わせて差異を確認する。

4. **フロントエンド(UI)の変更が含まれる場合**は、`Task`ツールで `ui-reviewer` サブエージェントを起動して UI 品質を検証する。
  - `subagent_type`: "ui-reviewer"
  - `description`: "UI design compliance review"
  - `prompt`: "今回実装した `[機能名]` の UI を検証してください。対象ファイルは `[UI関連ファイルのパスリスト]`、スクリーンショットは `[スクリーンショットのパスリスト]` です。docs/ui-design.md との整合、状態設計(loading/empty/error)の網羅、アクセシビリティ、レスポンシブを重点的に確認してください。"

**このステップが正常に完了したら、決して停止せず、ただちにステップ7に進むこと。**

## ステップ7: 自動テストの実行

1. 実装対象の言語に応じた品質チェックコマンドを順番に実行し、全てパスすることを確認する。使用するコマンドは `docs/development-guidelines.md` に定義されたものを優先する。
...
- ステップ6: `implementation-validator`サブエージェントの検証をパスする。フロントエンド(UI)の変更がある場合は、スクリーンショットによる視覚検証を行い、`ui-reviewer`サブエージェントの検証もパスする。
- ステップ7: 実装言語に応じた品質チェック（テスト・リント・型チェック）の全てのコマンドがエラーなく成功する。
