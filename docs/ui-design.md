...
flowchart TD
    ...
    S04 -->|投票完了、画面遷移せず結果待ち表示に切り替わる| S04
...
| success | 「投票ありがとうございました」Toast表示後、画面遷移はせず結果待ち表示に切り替わる（下記参照） | 送信成功 |
...
- 「投票する」タップ → 送信 → 成功Toast → 結果待ち表示に切り替わる（S-01への自動遷移はしない）

**結果待ち表示（投票完了後）**: 投票送信成功後は同一画面内でグリッドの代わりに結果待ちメッセージを表示し、5秒間隔でフェーズをポーリングして運営の進行に追従する。
...
| フェーズ | 表示メッセージ |
|----------|----------------|
| `voting`（投票直後） | 投票ありがとうございました。結果発表をお楽しみに🎉 |
| `results` | ただいま結果発表中です。会場の画面をご確認ください |
| `ended` | コンペは終了しました。ご参加ありがとうございました |
...
- 現在フェーズの表示
- フェーズ切り替えボタン群（4フェーズ分。投稿受付開始／投票開始／結果発表／コンペを終了する）
...
- AnimatedRankingList（画面固有、タイムラプス完了後のスライドイン＋カウントアップ演出用。`framer-motion`使用）
- Badge（同着表示用）
- Button（「発表開始」）
