...
| ID | 画面 | 概要 | 状態 |
| --- | --- | --- | --- |
| S-01 | トップ画面（`/c/[slug]`） | コンペ概要と投稿/投票への導線。初回アクセス時は参加者名入力ゲートを表示 | ✅ 実装済み |
| S-02 | 画像投稿画面 | ロゴ画像・一口メモを投稿（投稿者名は参加者名入力ゲートで取得済みの名前を自動使用） | ✅ 実装済み |
| S-03 | スワイプ1次選考画面 | 画像をランダム順に表示し、キープ／次へで仕分け | ✅ 実装済み |
| S-04 | 決選投票画面 | キープした画像から上位3つまで選んで投票。投票後は自動遷移せずフェーズに追従する結果待ち画面を表示 | ✅ 実装済み |
| S-05 | 管理者ログイン画面 | 運営が管理者機能にアクセスするための認証 | ✅ 実装済み |
| S-06 | コンペ管理画面（`/admin/competitions/[id]`） | 対象コンペのフェーズ切り替え（コンペ終了を含む）、QRコード表示、投稿状況・投票状況の確認（投稿状況は件数が多くてもスクロール領域に収まる） | ✅ 実装済み |
| S-07 | 結果発表画面（`/admin/competitions/[id]/results`） | 「発表開始」タップ後、投票を時系列に1票ずつ再生するタイムラプス演出を挟んでから、得票ランキングをアニメーション付きで表示（投票0件の場合はタイムラプスをスキップ）。`ended`（コンペ終了）に到達したコンペでも引き続き閲覧・CSVエクスポート可能 | ✅ 実装済み |
...
`0001`適用済みの既存プロジェクトでフェーズ`ended`（コンペ終了）を使えるようにするには、[`scripts/migrations/0002_add_ended_phase.sql`](scripts/migrations/0002_add_ended_phase.sql) を実行してください（`current_phase`のCHECK制約に`ended`を追加します。未適用の場合、コンペ管理画面の「コンペを終了する」操作がエラーになります）。新規プロジェクトでは`scripts/schema.sql`に最初から含まれているため不要です。
